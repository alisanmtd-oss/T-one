...
![T One 中文能力市场](assets/screenshots/t-one-capability-market-zh.png)
...
- 中文对话工作区：按项目长期保留经营上下文的产品方向演示。
- 中文能力市场：把 **Agent、Skill、MCP、CLI** 分成四类，再按用途筛选。
- 本地示例任务：可以把公开 Agent 和 Skill 加入示例任务，选择只保存在当前电脑。
...
- 私有商业连接器、电脑控制运行时和内部证据；
- 自动店铺写入、外联、广告花费、付款、发货或退款。

因此，界面中的 MCP 和 CLI 在公开离线版显示为“未配置”或“未检测”，不会用按钮冒充已经连接。
...
直接打开 `demo/chat-first-workspace.html` 可以查看离线中文演示。Windows 安装器源码位于 `desktop_public/`，固定 Electron 与 electron-builder 版本，可由公开 GitHub Actions 重复构建。
...
| `demo/` | 中文离线工作区和能力市场 |
| `desktop_public/` | Windows Electron 外壳与 NSIS 安装配置 |
| `config/` | 合成示例和公开安全配置 |
